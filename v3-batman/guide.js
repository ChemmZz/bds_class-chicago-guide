/* ============================================================
   CHICAGO GUIDE — guide.js  (V3 Gotham Noir)
...
function buildPopupHTML(spot, index) {
  ...
  const tagPills = spot.tags
    .map(t => `<span class="tag-pill" style="color:${TAG_COLORS_HEX[t]||'#f5c518'}">${t}</span>`)
    .join(" ");
  return `
    <div class="map-popup">
      <div class="map-popup__num" style="background:${colorHex};color:#000">${index}</div>
...
function updateMap(orderedSpots) {
  ...
  // Draw route polyline — bat signal yellow
...
function initTagButtons() {
  document.querySelectorAll(".tag-btn").forEach(btn => {
    ...
    btn.style.setProperty("--tag-color", TAG_COLORS_HEX[tag] || "#f5c518");
