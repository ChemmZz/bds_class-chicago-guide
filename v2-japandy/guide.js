/* ============================================================
   CHICAGO GUIDE v2 — Japandy — guide.js
...
/* v2: desaturated palette that works on light map tiles */
...
function resolveSpotColorHex(spot) {
  ...
  return "#3d5a80"; /* v2 ink-blue fallback */
}
...
function buildPopupHTML(spot, index) {
  ...
  const tagPills = spot.tags
    .map(t => `<span class="tag-pill" style="color:${TAG_COLORS_HEX[t]||'#3d5a80'}">${t}</span>`)
    .join(" ");
  return `
    <div class="map-popup">
      <div class="map-popup__num" style="color:${colorHex};border-color:${colorHex}">${index}</div>
...
function updateMap(orderedSpots) {
  ...
  orderedSpots.forEach((spot, i) => {
    const colorHex = resolveSpotColorHex(spot);
    /* v2: outline-circle markers — white interior lets basemap show through */
    const icon = L.divIcon({
      className: "",
      html: `<div class="route-marker" style="--marker-color:${colorHex}">${i + 1}</div>`,
      iconSize: [26, 26],
      iconAnchor: [13, 13],
      popupAnchor: [0, -15]
    });
  ...
  // Draw route polyline — v2: ink-blue, thinner, quieter
...
function createCardElement(spot) {
  ...
  /* v2: dot replaces left-border accent; no .spot-card__body wrapper */
  article.innerHTML = `
    <div class="spot-card__dot"></div>
...
function renderCards(orderedSpots) {
  ...
  const filteredIds = new Set(orderedSpots.map(s => s.id));
  ...
  // Re-order and show cards matching filter
  orderedSpots.forEach((spot, i) => {
...
function initTagButtons() {
  document.querySelectorAll(".tag-btn").forEach(btn => {
    ...
    btn.style.setProperty("--tag-color", TAG_COLORS_HEX[tag] || "#3d5a80");
